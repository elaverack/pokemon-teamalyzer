...
function OffensiveTableTool(props) {
  function calcDamage(type1, type2, poke, move) {
    ...
    let damageData = Object.keys(TYPE_CHART[gen]).map(type2 => {
      ...
      Object.keys(TYPE_CHART[gen]).map(type1 =>
        Object.defineProperty(row, type1, {
          value: props.teamState.team
            .map(pokemon =>
              pokemon.moves
                .map(move => calcDamage(type1, type2, pokemon, move))
